package com.xiongsu.core.markdown;
...
//此段代码的目的是 扩展 Flexmark Markdown 解析器，支持在 Markdown 文档中解析特定格式的提醒块（如警告、提示、注意等），
// 并将其渲染为适当的 AdmonitionBlock 对象。这些块可以在最终渲染时被转化为 HTML、PDF 等格式，显示不同的样式和图标。
