...
import com.xiongsu.service.article.conveter.ColumnArticleStructMapper;
import com.xiongsu.service.article.conveter.ColumnStructMapper;
...
public class ColumnSettingServiceImpl implements ColumnSettingService {
     ...
     */
    //保存专栏文章
    //主要是简单场景的处理：仅需要提供文章 ID 和专栏 ID 即可完成的保存逻辑
    @Override
    ...
    //更复杂场景的处理：使用封装好的请求对象 ColumnArticleReq，该对象可能包含文章的短标题 (shortTitle)、文章 ID、专栏 ID 等信息。
    @Override
    ...
    public void deleteColumnArticle(Long id) {
        ...
        if (columnArticleDO != null) {
            ...
            columnArticleDao.update(null, Wrappers.<ColumnArticleDO>lambdaUpdate()
                    ...
                    .eq(ColumnArticleDO::getColumnId, columnArticleDO.getColumnId())
                    //section 大于 1
                    .gt(ColumnArticleDO::getSection, 1)//确保 section 的值大于 1，避免更新到不合法的值（如负数或者 0）。
                    .gt(ColumnArticleDO::getSection, columnArticleDO.getSection()));//只更新 section 大于被删除文章的 section 的文章。
        }
    }
    ...
    public PageVo<ColumnDTO> getColumnList(SearchColumnReq req) {
       //转换参数
        ...
        //优化前代码
        // 假设 columnDTOS 是一个查询结果列表，包含多个 ColumnDTO 对象
      //  List<ColumnDTO> columnDTOS = getColumnDTOS();

        // 原来的做法是每次遍历 `columnDTOS`，对于每个 ColumnDTO，都单独查询用户信息
    //    for (ColumnDTO columnDTO : columnDTOS) {
            // 每次查询单独的用户信息（可能会发送多个数据库请求）
    //        BaseUserInfoDTO user = userService.queryUserInfoById(columnDTO.getAuthor());
    ...
    //    }
        ...
        if (CollUtil.isNotEmpty(columnDTOS)) {
            ...
            // 创建一个id到用户信息的映射
            //其中键是用户 ID，值是用户信息。这使得我们可以通过用户 ID 快速查找对应的用户信息。
    ...
    public void sortColumnArticleApi(SortColumnArticleReq req) {
        //根据req的两个ID调换两篇文章的顺序
        ColumnArticleDO activeDO = columnArticleDao.getById(req.getActiveId());//当前被拖动文章的原始 section（顺序位置）。
        ColumnArticleDO overDO = columnArticleDao.getById(req.getOverId());//是目标位置
        if (activeDO != null && overDO != null && !activeDO.getId().equals(overDO.getId())) {
            Integer activeSection = activeDO.getSection();//是当前被拖动文章的原始 section（顺序位置）
            Integer overSection = overDO.getSection();//是目标位置
            // 假如原始顺序为1、2、3、4
            //
