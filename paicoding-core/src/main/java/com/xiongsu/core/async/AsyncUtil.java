...
public class AsyncUtil {
    ...
    //这个 static 代码块的作用是在类加载时自动执行 initExecutorService 方法，初始化线程池或执行某些静态初始化操作。
    static {
    ...
    public static void initExecutorService(int core, int max) {
        ...
        executorService = new ExecutorBuilder()
                .setCorePoolSize(core)//核心线程池大小
                .setMaxPoolSize(max)// 最大线程池大小
                .setKeepAliveTime(0)// 线程池中空闲线程的存活时间
                .setKeepAliveTime(0, TimeUnit.SECONDS)
                .setWorkQueue(new SynchronousQueue<Runnable>()) //保存等待执行任务的队列
                .setHandler(new ThreadPoolExecutor.CallerRunsPolicy())//用来处理新提交任务的策略  ThreadPoolExecutor.CallerRunsPolicy() 表示任务会在调用者线程中运行，避免丢弃任务或抛出异常。
                .setThreadFactory(THREAD_FACTORY)//创建新线程的工厂方法
                .buildFinalizable();//这个方法似乎是用来处理线程池生命周期的某些最终处理，确保线程池在使用完之后能够被正确回收。这个不是线程池的标准参数，但可以理解为一种自定义的增强配置。
        //包装一下线程池，避免出现上下文复用场景
