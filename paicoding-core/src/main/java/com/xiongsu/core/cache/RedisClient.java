...
public class RedisClient {
    ...
    public static Boolean setStrWithExpire(String key, String value, Long expire) {
        //1. template.execute(RedisCallback<T>)
        //template 这里一般是 RedisTemplate 实例，用于操作 Redis。
        //.execute(new RedisCallback<Boolean>() { ... }) 是 Spring 提供的回调机制，让你直接访问 RedisConnection（底层 Redis 连接）。
        //这可以 避免序列化问题，也可以 减少 Redis 连接的开销，提高性能。
        return template.execute(new RedisCallback<Boolean>() {
            @Override
            //setEx 方法：表示 SET key value EX seconds，即存入 Redis 并设置过期时间。
            //keyBytes(key)：将 String 类型的 key 转换为 byte[]（Redis 存储的是二进制数据）。
            //valBytes(value)：将 String 类型的 value 转换为 byte[]。
            //expire：过期时间，单位是秒。
