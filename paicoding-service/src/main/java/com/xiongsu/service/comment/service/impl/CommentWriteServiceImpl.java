...
public class CommentWriteServiceImpl implements CommentWriteService {
    ...
    public void deleteComment(Long commentId, Long userId) {
        ...
        //3.发布删除评论事件
        SpringUtil.publishEvent(new NotifyMsgEvent<>(this, NotifyTypeEnum.DELETE_COMMENT, commentDO));//这条评论被删除
        if (NumUtil.upZero(commentDO.getParentCommentId())) {
            // 评论
            SpringUtil.publishEvent(new NotifyMsgEvent<>(this, NotifyTypeEnum.DELETE_REPLY, commentDO));//这条回复被删除
        }
    }
